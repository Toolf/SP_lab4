...
{
    this->last_taken = Token(UNKNOWN, "", 1, 1);
    this->code = code;
    this->index = 0;
    this->row = 1;
    this->col = 1;
...
Token CLexer::nextToken()
{
    ...
    {
        Token t = get_int_or_float(code, this);
        this->last_taken = t;
        return t;
    }
    // var or keyword
    ...
    {
        Token t = get_var_or_keyword(code, this);
        this->last_taken = t;
        return t;
    }
    // other
    else
    {
        ...
        while (char_count > 0)
        {
            ...
            if (it != tokenTypes.end())
            {
                ...
                Token t = Token(it->second, value, this->row, col);
                this->last_taken = t;
                return t;
            }
            char_count--;
        ...
        Token t = Token(UNKNOWN, this->code.substr(this->index - 1, 1), this->row, this->col - 1);
        this->last_taken = t;
        return t;
    }
    throw LexerException("EOF");
